...
public class AlignRobotToTargetCommand extends CommandBase {
    ...
    public AlignRobotToTargetCommand(
            SwerveSubsystem drivetrain, LimeLightSubSystem vision, DoubleSupplier xAxis, DoubleSupplier yAxis) {
    ...
    public void execute() {
        if (targetSeen) {
            ...
            controller.setSetpoint(
                    vision.getTargetPose3d_RobotSpace().toPose2d().getRotation().getRadians());
            ...
            rotationalVelocity += Math.copySign(
                    ROTATION_STATIC_CONSTANT / MAX_VOLTAGE * MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND,
                    rotationalVelocity);
