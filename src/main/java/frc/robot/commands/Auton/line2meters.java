...
public class line2meters extends SequentialCommandGroup {
    ...
    public Command loadPathPlannerTrajectoryCommand(String filename, boolean resetOdometry) {
        ...
        if (resetOdometry) {
            return new SequentialCommandGroup(
                    new InstantCommand(
                            () -> RobotContainer.getSwerveSubsystem().resetOdometry(trajectory.getInitialPose())),
    ...
    public line2meters(SwerveSubsystem swerveSubsystem) {

        addCommands(loadPathPlannerTrajectoryCommand("line2meters", true));
    }
}
