  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(coDriverJoystick, OIConstants.kDriverButtonY).onTrue(new InstantCommand(() -> armSubsystem.setArmPositionState(ArmPosition.HighDrop))).onTrue(
      ...
      new Trigger(() -> coDriverJoystick.getRawAxis(OIConstants.kDriverYAxis) > 0.05 || coDriverJoystick.getRawAxis(OIConstants.kDriverYAxis) < -0.05).onTrue(
        new MoveArmCommand(armSubsystem, ArmConstants.scoreGroundPosition.get("armCmdPos") + coDriverJoystick.getRawAxis(OIConstants.kDriverRotYAxis) * ArmConstants.wristAdjustMultiplier,
          ArmConstants.scoreGroundPosition.get("smartMotionMaxVel"),
          ArmConstants.scoreGroundPosition.get("smartMotionMaxAccel"))
      );
  }
